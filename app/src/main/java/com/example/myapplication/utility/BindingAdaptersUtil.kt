...
object BindingAdaptersUtil {
    ...
    fun setImageSelector(view: ImageView, isSelected: Boolean) {
        if (isSelected) OfferViewUtil.setImageButtonSelector(view)
    }

    @JvmStatic
    @BindingAdapter(
        value = ["android:setBounceSelector", "android:showShadow", "android:justElevate", "android:needRounded"], requireAll = false
    )
    fun setBounceSelector(
        ...
        if (showShadow !== null && showShadow) {
            ...
            view.z = OfferViewUtil.dipToPixel(view, 4).toFloat()
    ...
    fun setTextViewSelector(view: View, isSelected: Boolean) {
        if (isSelected) OfferViewUtil.setTextViewSelector(view)
    }
    ...
    @BindingAdapter(
        value = ["android:setFormattedPricePrefix", "android:setFormattedPrice", "android:setFormattedPriceSuffix"], requireAll = false
    )
...
//        } else {
        textView!!.text = String.format("$setFormattedPricePrefix $setFormattedPrice $setFormattedPriceSuffix")
//        }
    }
    ...
    public fun loadMedia(url: Any?, placeHolder: Drawable?, imageView: ImageView) {
        if (url!! == "dummy_amazon") {
            ...
            imageView.post {
                getRequestOption(placeHolder, imageView.width, imageView.height)?.let {
                    Glide.with(imageView.context.applicationContext).load(url).apply(it).centerCrop()
                        .into(imageView)
                }
            }
        }
    ...
    fun getRequestOption(resId: Drawable?, width: Int, height: Int): RequestOptions? {
        return RequestOptions().override(width, height).placeholder(resId).error(resId).fallback(resId).dontAnimate()
...
//        } else {
//            view.imageTintList = ColorStateList.valueOf(ContextCompat.getColor(context, R.color.offer_text_color))
//        }
//    }
//
//    @JvmStatic
...
//                } else {
//                    view.tabTextColors = ColorStateList.valueOf(ContextCompat.getColor(context, R.color.offer_text_color))
//                    view.setSelectedTabIndicatorColor(ContextCompat.getColor(context, R.color.offer_text_color))
//                }
//            }
//        }
//    }
...
//    }
    ...
    @JvmStatic
    @BindingAdapter(
        value = ["android:pStartColor", "android:pEndColor", "android:pTopLeftCorner", "android:pTopRightCorner", "android:pBottomLeftCorner", "android:pBottomRightCorner", "android:shapeOrientation", "android:isStroke"], requireAll = false
    )
    fun setBackgroundShape(
        ...
        view.background = GradeDrawable(
            Color.parseColor(pStartColor ?: "#0012a269"), Color.parseColor(
                ...
                ?: Orientation.TOP_BOTTOM, isStroke ?: false
        )
    }
    ...
    class GradeDrawable(
        ...
        init {
            shape = RECTANGLE
            cornerRadii = floatArrayOf(pTopLeftCorner, pTopLeftCorner, pTopRightCorner, pTopRightCorner, pBottomLeftCorner, pBottomLeftCorner, pBottomRightCorner, pBottomRightCorner)
