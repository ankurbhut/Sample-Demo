...
object OfferViewUtil {
                    ...
                    when (event?.action) {
                        MotionEvent.ACTION_DOWN -> {
                            ...
                            val anim: Animation = ScaleAnimation(
                                1f, 0.9f, 1f, 0.9f, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f
                            )
                            anim.fillAfter = true
                            anim.duration = 70
                        ...
                        MotionEvent.ACTION_MOVE -> {
                            if (rect != null) {
                                if (!rect?.contains(
                                    ...
                                    val anim: Animation = ScaleAnimation(
                                        0.9f, 1f, 0.9f, 1f, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f
                                    )
                                    anim.fillAfter = true
                                    anim.duration = 70
                                    view.startAnimation(anim)
        ...
        for (imageView in views) {
            imageView.setOnTouchListener(object : View.OnTouchListener {
                ...
                override fun onTouch(view: View, event: MotionEvent): Boolean {
                    when (event.action) {
                        MotionEvent.ACTION_DOWN -> {
                            rect = Rect(imageView.left, imageView.top, imageView.right, imageView.bottom)
                            if (imageView.background != null) {
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                                    imageView.background.colorFilter = BlendModeColorFilter(0x44000000, BlendMode.SRC_ATOP)
                                } else {
                                    @Suppress("DEPRECATION") imageView.background.setColorFilter(0x44000000, PorterDuff.Mode.SRC_ATOP)
                                }
                                imageView.invalidate()
                            }
                        }
                        MotionEvent.ACTION_MOVE -> {
                            rect?.let {
                                if (!it.contains(imageView.left + event.x.toInt(), imageView.top + event.y.toInt())) { // User moved outside bounds
                                    if (imageView.background != null) {
    ...
    fun appReferralSharing(activity: FragmentActivity?, referralLink: String, appName: String, userName: String) {
        justTry {
...
//        }
    ...
    fun getApplicationInstallDate(context: Context): Int {
        val elapsedTimestamp: Long = context.packageManager.getPackageInfo(context.packageName, 0).firstInstallTime
